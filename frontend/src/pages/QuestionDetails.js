...
const ANSWER_ALLOWED_REGEX = /^[A-Za-z0-9 ]+$/;
...
const QuestionDetails = () => {
  ...
  const handleAnswerSubmit = async (e) => {
    ...
    try {
      await questionService.createAnswer(questionId, { content: normalizedAnswer });
  ...
  const handleAnswerContentChange = (e) => {
    ...
    if (normalizedValue && !ANSWER_ALLOWED_REGEX.test(normalizedValue)) {
      setAnswerError('Add Your Answer accepts only English letters, numbers, and spaces');
      return;
    }
  ...
  const handleMarkBestAnswer = async (answerId) => {
    try {
      await questionService.markAnswerAccepted(answerId);
      fetchQuestion();
      toast.success('Best answer updated');
    } catch (error) {
      const message = error?.response?.data?.message || 'Failed to update best answer';
      toast.error(message);
    }
  };
    ...
    <div style={{ ...styles.container, marginTop: '30px' }}>
      ...
      <div style={{ ...styles.card, marginBottom: '20px' }}>
        ...
        <div style={{ display: 'flex', gap: '10px', marginTop: '10px', flexWrap: 'wrap' }}>
          ...
          {question.subject?.name && (
            <button type="button" onClick={() => navigate(`/questions?subject=${question.subject._id}`)} style={{ ...styles.button, padding: '4px 10px' }}>
              {question.subject.name}
            </button>
          )}
          {question.topic?.name && (
            <button type="button" onClick={() => navigate(`/questions?subject=${question.subject?._id || ''}&topic=${question.topic._id}`)} style={{ ...styles.button, padding: '4px 10px' }}>
              #{question.topic.name}
            </button>
          )}
          <span style={{ ...styles.badge, ...styles.badgePrimary }}>
            Posted: {getTimeLabel(question.createdAt)}
          </span>
        </div>
      </div>
        ...
        {sortedAnswers.map((answer) => (
          <div
            ...
            style={{
              ...styles.card,
              marginTop: '10px',
              border: answer.isAccepted ? '2px solid #16a34a' : '1px solid #e2e8f0',
              background: answer.isAccepted ? '#f0fdf4' : '#ffffff'
            }}
          >
            ...
            {answer.isAccepted && (
              <div style={{ marginBottom: '10px' }}>
                <span style={{ ...styles.badge, ...styles.badgeSuccess }}>Best Answer</span>
              </div>
            )}

            <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap', alignItems: 'center' }}>
              <span style={{ ...styles.badge, ...(answer.isAccepted ? styles.badgeSuccess : styles.badgePrimary) }}>
                {answer.isAccepted ? 'Highlighted' : 'Answer'}
              </span>
              <span style={{ ...styles.badge, ...styles.badgePrimary }}>
                By: {answer.answerer?.name || 'Unknown'}
              </span>
              <span style={{ ...styles.badge, ...styles.badgePrimary }}>
                Likes: {answer.helpfulCount || 0}
              </span>
              <span style={{ ...styles.badge, ...styles.badgePrimary }}>
                Posted: {getTimeLabel(answer.createdAt)}
              </span>
              ...
              {question.asker?._id === user?.id && (
                <button
                  ...
                  style={{ ...styles.button, padding: '6px 12px', background: answer.isAccepted ? '#16a34a' : '#0f766e' }}
                >
                  {answer.isAccepted ? 'Best Answer' : 'Mark as Best Answer'}
                </button>
              )}
      ...
      <div style={styles.card}>
        ...
        <form onSubmit={handleAnswerSubmit}>
          ...
          />
          {answerError && (
            <p style={{ margin: '6px 0 10px 0', color: '#dc2626', fontSize: '13px' }}>{answerError}</p>
          )}
