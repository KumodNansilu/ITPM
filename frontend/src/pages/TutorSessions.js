...
const TutorSessions = () => {
  ...
  const [createFormErrors, setCreateFormErrors] = useState({ topicText: '' });
  ...
  const handleCreateSession = async (e) => {
    ...
    if (createForm.sessionDate < todayLocalDate) {
      toast.error('Session date cannot be before today');
      return;
    }

    const trimmedTopic = createForm.topicText.trim();
    if (trimmedTopic && !englishLettersAndNumbersRegex.test(trimmedTopic)) {
      setCreateFormErrors({ topicText: 'Lesson/Topic can only contain English letters and numbers.' });
      return;
    }

    try {
      const dateTime = new Date(`${createForm.sessionDate}T${createForm.sessionTime}`);
      const finalDescription = trimmedTopic
        ? `Lesson/Topic: ${trimmedTopic}${createForm.description ? `\n\n${createForm.description}` : ''}`
        : createForm.description;
  ...
  const handleDeleteSession = async () => {
    const confirmed = await confirmDialog({
      ...
      text: 'This will permanently delete the session and all related bookings. Continue?',
      icon: 'warning',
    ...
    try {
      await appointmentService.deleteTutorSession(selectedSession._id);
      toast.success('Session deleted successfully');
      fetchTutorSessions();
      setSelectedSession(null);
    } catch (error) {
      if (error?.response?.status === 404) {
        toast.error('Delete endpoint not found. Please restart backend server and try again.');
        return;
      }
      toast.error(error.response?.data?.message || error.message || 'Failed to delete session');
    }
  };
  ...
  const sessionsBySubject = useMemo(() => {
    const grouped = (sessions || []).reduce((acc, session) => {
      ...
      acc[subjectName].push(session);
      return acc;
    }, {});

    return Object.entries(grouped).sort(([a], [b]) => a.localeCompare(b));
  }, [sessions]);
  const filteredSessionsBySubject = useMemo(() => {
    const query = subjectSearchQuery.trim().toLowerCase();
    if (!query) return sessionsBySubject;

    return sessionsBySubject.filter(([subjectName]) =>
      subjectName.toLowerCase().includes(query)
    );
  }, [sessionsBySubject, subjectSearchQuery]);
          ...
          <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap', marginTop: '16px', marginBottom: '18px' }}>
            {selectedSession.status === 'scheduled' && (
              <>
                ...
                <button onClick={handleDeleteSession} style={styles.buttonDanger}>Delete</button>
  ...
  return (
    <div style={{ ...styles.container, marginTop: '30px' }}>
      ...
      {activeTab === 'sessions' && (
        <>
          <div style={{ ...styles.card, marginBottom: '14px', padding: '14px' }}>
            <label style={{ ...styles.label, marginBottom: '8px', display: 'block' }}>Search Subject</label>
            <input
              type="text"
          ...
          {sessions.length > 0 && filteredSessionsBySubject.length === 0 && (
            <div style={styles.alertInfo}>No subjects match your search.</div>
          )}

          {filteredSessionsBySubject.map(([subjectName, subjectSessions]) => (
            <section key={subjectName} style={{ marginBottom: '22px' }}>
              <h2 style={{ margin: '0 0 12px 0', color: '#0b1f3b' }}>
                {subjectName} ({subjectSessions.length})
              </h2>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: '15px' }}>
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => {
                    const rawTopicText = e.target.value;
                    const cleanedTopicText = sanitizeToEnglishLettersAndNumbers(rawTopicText);
                    setCreateForm((p) => ({ ...p, topicText: cleanedTopicText }));
                    setCreateFormErrors({
                      topicText: rawTopicText !== cleanedTopicText
                        ? 'Only English letters and numbers are allowed.'
                        : ''
                    });
                  }}
                  onBlur={() => {
                    const trimmedTopicText = createForm.topicText.trim();
                    setCreateFormErrors({
                      topicText: trimmedTopicText && !englishLettersAndNumbersRegex.test(trimmedTopicText)
                        ? 'Lesson/Topic can only contain English letters and numbers.'
                        : ''
                    });
                  }}
                  placeholder="Type lesson or topic name"
                />
                {createFormErrors.topicText && (
